...
Provide clear, step-by-step explanations. Include the final answer at the end.`
    }, memory = aiShorttermmemory, model = mathTutorModel, tools = [sumTool, subtractTool, multiplyTool, divideTool, messageToAdmin, reportuserInfo], verbose = false
);
...
@ai:AgentTool
isolated function messageToAdmin() returns string {
    io:println("$$$$$$$$$$$$$$");
    return "successfully sent message";
}

@ai:AgentTool
isolated function reportuserInfo(UserInfo UserInfo) returns string {
    io:println(UserInfo);
    return "reported";
}
